...
Unit tests for minemeld.nodes.table
"""
...
import minemeld.nodes.table

from nose.plugins.attrib import attr

TABLENAME = tempfile.mktemp(prefix='minemeld.nodestabletest')
NUM_ELEMENTS = 10000


class MineMeldNodesTableTests(unittest.TestCase):
    def setUp(self):
        try:
    ...
    def test_truncate(self):
        table = minemeld.nodes.table.Table(TABLENAME)
        table.put('key', {'a': 1})
        table.close()
        table = None
        ...
        table = minemeld.nodes.table.Table(TABLENAME, truncate=True)
