...
class ZMQRpcServerChannel(object):
    ...
    def run(self):
        ...
        while True:
            LOG.debug('RPC Server recving from {} - {}'.format(self.name, self.fanout))
    ...
    def connect(self, context):
        ...
        if self.fanout is not None:
            ...
            self.socket.bind('ipc:///var/run/minemeld/{}:rpc'.format(self.name))
...
class ZMQRedis(object):
    ...
    def send_rpc(self, dest, method, params,
        ...
        socket.connect('ipc:///var/run/minemeld/{}:rpc'.format(dest))
